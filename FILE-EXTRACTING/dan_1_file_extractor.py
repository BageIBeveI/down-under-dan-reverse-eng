...
if not os.path.exists(daninput):
    os.makedirs(daninput)
if not os.path.exists(danoutput):
...
for danfile in ['1TO3.1', '3TO6.1', '6TO9.1', '9TO12.1', '12TO15.1', '15TO18.1', '18TO21.1', 'BACK.1', 'IM.1', 'MISC.1', 'MUST.1', 'OBJVOC.1', 'PLTA.1', 'RIVA.1', 'RIVB.1', 'SHER.1', 'SO.1']: #big loop for every dan .1 file
    try:
        ...
        if not os.path.exists(f"{danoutput}/"+ str(danfile[0:-2])):
            os.makedirs(f"{danoutput}/"+ str(danfile[0:-2]))     #makes the folder for the files in its own directory instead of cluttering current folder.
        ...
        for k in range(0, len(file_names)):                 #for however many subfiles are in the .1 file,
            ...
            if file_names[k][-1:] in "2345":
                badFlag = True
            if badFlag:
                file_dir_and_name = f"{danoutput}/" + str(danfile[0:-2] + "/" + file_names[k][0:-4] + "-" + file_names[k][-1] + file_names[k][-4:-1] + file_names[k][-1])
            else:
                file_dir_and_name = f"{danoutput}/" + str(danfile[0:-2] + "/" + file_names[k])     #store the directory with file name for each subfile in this variable
